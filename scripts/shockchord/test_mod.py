...
Usage:
    ...
    python evaluate_gparc_model.py --model_path best_model.pth --test_dir /path/to/test --output_dir ./evaluation
    
    # Test specific simulation files:
    python evaluate_gparc_model.py --model_path best_model.pth --test_files sim1.pt sim2.pt sim3.pt --output_dir ./evaluation
"""
...
################################################################################
# ENHANCED ROLLOUT EVALUATOR WITH DELTA_T ANALYSIS
################################################################################

class GPARCRolloutEvaluator:
    """
    Evaluator for GPARC models using rollout prediction mode.
    Enhanced with delta_t analysis and improved filename generation.
    """
    ...
    def generate_rollout(self, initial_data, rollout_steps):
        ...
        Matches training forward() logic.
        """
        predictions = []
        F_prev = None
    ...
    def analyze_multi_parameter_performance(self):
        """Analyze performance across all global parameters (delta_t, pressure, density)."""
        if not self.delta_t_metrics:
            return {}
        
        # Collect all simulation metadata with performance metrics
        multi_param_data = []
        
        # We need to reconstruct the full parameter space from our tracking
        for delta_t_str, metrics_list in self.delta_t_metrics.items():
            for sim_metrics in metrics_list:
                # Find corresponding simulation metadata
                sim_data = {
                    'delta_t': float(delta_t_str),
                    'delta_t_str': delta_t_str,
                    'performance_metrics': sim_metrics
                }
                multi_param_data.append(sim_data)
        
        return multi_param_data
    
    def create_multi_parameter_analysis(self, metadata):
        """Create comprehensive multi-parameter analysis from simulation metadata."""
        if not metadata:
            return {}
        
        # Extract all parameter combinations and their performance
        param_combinations = []
        
        for sim_meta in metadata:
            # Get denormalized parameter values
            pressure = sim_meta['pressure'] 
            density = sim_meta['density']
            delta_t = sim_meta['delta_t_physical']
            
            param_combination = {
                'delta_t': delta_t,
                'pressure': pressure, 
                'density': density,
                'case_name': sim_meta['case_name'],
                'simulation_idx': sim_meta['simulation_idx']
            }
            param_combinations.append(param_combination)
        
        return param_combinations
    
    def plot_3d_parameter_performance(self, predictions, targets, metadata, figsize=(20, 15)):
        """Create 3D scatter plots showing performance across all three global parameters."""
        ...
        for sim_idx, (pred_seq, targ_seq, sim_meta) in enumerate(zip(predictions, targets, metadata)):
            # Calculate overall R² and RMSE for this simulation
            all_preds = []
            all_targs = []
            
            for step_pred, step_targ in zip(pred_seq, targ_seq):
                all_preds.append(step_pred.cpu().numpy())
                all_targs.append(step_targ.cpu().numpy())
        ...
        if not sim_performances:
            return None
        
        # Convert to arrays for plotting
        delta_ts = np.array([s['delta_t'] for s in sim_performances])
        pressures = np.array([s['pressure'] for s in sim_performances]) 
        densities = np.array([s['density'] for s in sim_performances])
        r2_scores = np.array([s['r2'] for s in sim_performances])
        rmse_scores = np.array([s['rmse'] for s in sim_performances])
        
        # Create 3D plots
        fig = plt.figure(figsize=figsize)
        
        # 3D scatter plot - R² colored by performance
        ax1 = fig.add_subplot(221, projection='3d')
        scatter = ax1.scatter(delta_ts, pressures, densities, c=r2_scores, 
                            cmap='viridis', s=60, alpha=0.8)
        ax1.set_xlabel('Delta_t (s)')
        ax1.set_ylabel('Pressure')
        ax1.set_zlabel('Density')
        ax1.set_title('R² Performance in Parameter Space')
        plt.colorbar(scatter, ax=ax1, label='R² Score', shrink=0.8)
        
        # Format delta_t axis
        ax1.xaxis.set_major_formatter(plt.FuncFormatter(lambda x, p: f'{x:.1e}'))
        
        # 3D scatter plot - RMSE colored by performance  
        ax2 = fig.add_subplot(222, projection='3d')
        scatter2 = ax2.scatter(delta_ts, pressures, densities, c=rmse_scores,
                             cmap='plasma', s=60, alpha=0.8)
        ax2.set_xlabel('Delta_t (s)')
        ax2.set_ylabel('Pressure') 
        ax2.set_zlabel('Density')
        ax2.set_title('RMSE in Parameter Space')
        plt.colorbar(scatter2, ax=ax2, label='RMSE', shrink=0.8)
        ax2.xaxis.set_major_formatter(plt.FuncFormatter(lambda x, p: f'{x:.1e}'))
        
        # 2D projections
        # Delta_t vs Pressure (colored by R²)
        ax3 = fig.add_subplot(223)
        scatter3 = ax3.scatter(delta_ts, pressures, c=r2_scores, cmap='viridis', s=60, alpha=0.8)
        ax3.set_xlabel('Delta_t (s)')
        ax3.set_ylabel('Pressure')
        ax3.set_title('R² Performance: Delta_t vs Pressure')
        ax3.xaxis.set_major_formatter(plt.FuncFormatter(lambda x, p: f'{x:.1e}'))
        plt.colorbar(scatter3, ax=ax3, label='R² Score')
        
        # Pressure vs Density (colored by R²)
        ax4 = fig.add_subplot(224)
        scatter4 = ax4.scatter(pressures, densities, c=r2_scores, cmap='viridis', s=60, alpha=0.8)
        ax4.set_xlabel('Pressure')
        ax4.set_ylabel('Density')
        ax4.set_title('R² Performance: Pressure vs Density') 
        plt.colorbar(scatter4, ax=ax4, label='R² Score')
        
        plt.tight_layout()
        return fig
    
    def plot_parameter_correlation_analysis(self, predictions, targets, metadata, figsize=(18, 12)):
        ...
        for sim_idx, (pred_seq, targ_seq, sim_meta) in enumerate(zip(predictions, targets, metadata)):
            ...
            for i, var_name in enumerate(self.var_names):
                pred_var = all_preds[:, i]
                target_var = all_targs[:, i]
                var_r2 = r2_score(target_var, pred_var)
                var_rmse = np.sqrt(mean_squared_error(target_var, pred_var))
                var_metrics[f'{var_name}_r2'] = var_r2
                var_metrics[f'{var_name}_rmse'] = var_rmse
        ...
        param_cols = ['delta_t', 'pressure', 'density']
        metric_cols = [col for col in df.columns if col not in param_cols]
        ...
        fig.suptitle('Parameter-Performance Correlation Analysis', fontsize=16)
        
        # Correlation heatmap
        ax = axes[0, 0]
        im = ax.imshow(correlations.values, cmap='RdBu_r', aspect='auto', vmin=-1, vmax=1)
        ...
        # Individual parameter vs overall R² scatter plots
        for i, param in enumerate(['delta_t', 'pressure', 'density']):
            row = i // 3
            col = (i % 3) + 1 if i < 2 else 1
            ...
            ax.scatter(df[param], df['overall_r2'], alpha=0.7, s=50)
            ax.set_xlabel(param.title())
            ax.set_ylabel('Overall R²')
            ax.set_title(f'R² vs {param.title()}')
        ...
        ax = axes[1, 1]
        ax.hist(df['delta_t'], bins=20, alpha=0.7, label='Delta_t')
        ax.set_xlabel('Delta_t (s)')
        ax.set_ylabel('Count')
        ax.set_title('Delta_t Distribution')
        ax.xaxis.set_major_formatter(plt.FuncFormatter(lambda x, p: f'{x:.1e}'))
        
        ax = axes[1, 2]
        ax.hist2d(df['pressure'], df['density'], bins=10, alpha=0.7)
        ax.set_xlabel('Pressure')
        ax.set_ylabel('Density')
        ax.set_title('Pressure vs Density Distribution')
    ...
    def plot_delta_t_performance_analysis(self, figsize=(20, 12)):
        """Create comprehensive plots showing performance vs delta_t."""
        delta_t_analysis = self.analyze_delta_t_performance()
        
        if not delta_t_analysis:
            print("No delta_t analysis data available for plotting.")
            return None
        ...
        var_r2_data = {var_name: {'means': [], 'stds': []} for var_name in self.var_names}
        var_rmse_data = {var_name: {'means': [], 'stds': []} for var_name in self.var_names}
        ...
        for delta_t_str, analysis in sorted_items:
            ...
            for var_name in self.var_names:
                if var_name in analysis['variables']:
                    var_r2_data[var_name]['means'].append(analysis['variables'][var_name]['r2']['mean'])
                    var_r2_data[var_name]['stds'].append(analysis['variables'][var_name]['r2']['std'])
                    var_rmse_data[var_name]['means'].append(analysis['variables'][var_name]['rmse']['mean'])
                    var_rmse_data[var_name]['stds'].append(analysis['variables'][var_name]['rmse']['std'])
                else:
                    var_r2_data[var_name]['means'].append(0)
                    var_r2_data[var_name]['stds'].append(0)
                    var_rmse_data[var_name]['means'].append(0)
                    var_rmse_data[var_name]['stds'].append(0)
        ...
        fig.suptitle('Model Performance vs Physical Delta_t (seconds)', fontsize=16)
        
        # Overall R² vs delta_t - Scatter plot instead of connected line
        ax = axes[0, 0]
        ax.errorbar(delta_t_values, overall_r2_means, yerr=overall_r2_stds, 
                   fmt='o', markersize=8, capsize=5, capthick=2, linewidth=0, 
                   elinewidth=2, color='blue', alpha=0.7)
        ax.set_xlabel('Physical Delta_t (seconds)')
        ax.set_ylabel('R² Score')
        ax.set_title('Overall Model Performance (R²)')
        ax.grid(True, alpha=0.3)
        
        # Custom tick formatting for very small values
        ax.xaxis.set_major_formatter(plt.FuncFormatter(lambda x, p: f'{x:.2e}'))
        
        # Overall RMSE vs delta_t - Scatter plot
        ax = axes[0, 1]
        ax.errorbar(delta_t_values, overall_rmse_means, yerr=overall_rmse_stds,
        ...
        # Variable-specific R² vs delta_t - Scatter plots with different markers
        ax = axes[1, 0]
        colors = plt.cm.Set1(np.linspace(0, 1, len(self.var_names)))
        markers = ['o', 's', '^', 'D']  # Different markers for each variable
        
        for i, var_name in enumerate(self.var_names):
            marker = markers[i % len(markers)]
            ax.errorbar(delta_t_values, var_r2_data[var_name]['means'], 
                       yerr=var_r2_data[var_name]['stds'],
                       fmt=marker, markersize=6, capsize=3, linewidth=0,
                       elinewidth=1.5, label=var_name.title(),
                       color=colors[i], alpha=0.8)
        ax.set_xlabel('Physical Delta_t (seconds)')
        ax.set_ylabel('R² Score')
        ax.set_title('Variable-Specific Performance (R²)')
        ax.grid(True, alpha=0.3)
        ax.legend()
        ax.xaxis.set_major_formatter(plt.FuncFormatter(lambda x, p: f'{x:.2e}'))
        
        # Variable-specific RMSE vs delta_t - Scatter plots
        ax = axes[1, 1]
        for i, var_name in enumerate(self.var_names):
            marker = markers[i % len(markers)]
            ax.errorbar(delta_t_values, var_rmse_data[var_name]['means'],
                       yerr=var_rmse_data[var_name]['stds'],
                       fmt=marker, markersize=6, capsize=3, linewidth=0,
                       elinewidth=1.5, label=var_name.title(),
                       color=colors[i], alpha=0.8)
        ax.set_xlabel('Physical Delta_t (seconds)')
        ax.set_ylabel('RMSE')
        ax.set_title('Variable-Specific Error (RMSE)')
        ax.grid(True, alpha=0.3)
        ax.legend()
        ax.xaxis.set_major_formatter(plt.FuncFormatter(lambda x, p: f'{x:.2e}'))
        ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, p: f'{x:.2e}'))
    ...
    def plot_prediction_vs_target_scatter(self, predictions, targets, figsize=(15, 10)):
        ...
        for i, var_name in enumerate(self.var_names):
            ...
            pred_phys = self.denormalize_predictions(pred_sample, i)
            target_phys = self.denormalize_predictions(target_sample, i)
        ...
        if len(self.var_names) == 3 and len(axes) > 3:
...
################################################################################
# UTILITY FUNCTIONS (UPDATED FOR DELTA_T)
################################################################################
...
################################################################################
# MAIN EVALUATION FUNCTION (ENHANCED WITH DELTA_T ANALYSIS)
################################################################################
...
def evaluate_gparc_rollout(model_path, test_dir, test_files, output_dir, args):
   ...
   # The global embedding dimension from GlobalParameterProcessor is 64
    ...
    # Multi-parameter 3D analysis 
    print("Creating 3D parameter space analysis...")
    fig_3d = evaluator.plot_3d_parameter_performance(predictions, targets, metadata)
    if fig_3d:
        fig_3d.savefig(output_path / '3d_parameter_performance_analysis.png', 
                       dpi=300, bbox_inches='tight')
        plt.close(fig_3d)
    
    # Parameter correlation analysis
    print("Creating parameter correlation analysis...")
    correlation_result = evaluator.plot_parameter_correlation_analysis(predictions, targets, metadata)
    if correlation_result:
        fig_corr, correlation_matrix = correlation_result
        fig_corr.savefig(output_path / 'parameter_correlation_analysis.png', 
                        dpi=300, bbox_inches='tight')
        plt.close(fig_corr)
        
        # Save correlation matrix as CSV
        correlation_matrix.to_csv(output_path / 'parameter_performance_correlations.csv')
    
    # Delta_t performance analysis plot (original)
    fig_delta_t = evaluator.plot_delta_t_performance_analysis()
    if fig_delta_t:
        fig_delta_t.savefig(output_path / 'delta_t_performance_analysis.png', 
                           dpi=300, bbox_inches='tight')
        plt.close(fig_delta_t)
    
    # Static plots for selected simulations (now with delta_t in filenames)
