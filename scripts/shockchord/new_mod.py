...
from integrator.integrator import ExplicitIntegralGNN as IntegralGNN
...
def add_solver_arguments(parser):
    ...
    # NEW: Add argument for delta_t usage
    parser.add_argument("--use_direct_delta_t", action="store_true", default=True,
                        help="Use direct delta_t in integrator for interpretability")
...
def create_model_with_configurable_solvers(args):
    ...
    # The global embedding dimension from GlobalParameterProcessor
    # Changes based on whether we use direct delta_t or not
    if args.use_direct_delta_t:
        global_embed_dim = 64  # Only pressure and density embedded
    else:
        global_embed_dim = 64  # All three parameters embedded (original behavior)
    ...
    # NEW: Pass the use_direct_delta_t flag to GPARC
    model = GPARC(
        ...
        use_direct_delta_t=args.use_direct_delta_t  # NEW parameter
    )
...
def main():
    ...
    # Add solver-specific arguments (now includes use_direct_delta_t)
    ...
    test_dataset = ShockTubeRolloutDataset(
        directory=test_dir,
    ...
    logger.info(f"  Using direct delta_t: {args.use_direct_delta_t}")  # NEW log line
    ...
    with open(output_dir / "model_summary.txt", "w") as f:
        ...
        f.write(f"\n  Using direct delta_t: {args.use_direct_delta_t}")  # NEW line
