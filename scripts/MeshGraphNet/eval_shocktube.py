#!/usr/bin/env python3
"""
MeshGraphNet Shock Tube Evaluation Script
==========================================
Evaluates MeshGraphNet models on shock tube test data.
Mirrors eval_mgkan_shocktube.py — only model loading and forward pass differ.
...
def extract_global_params(data, device=None):
    parts = []
...
class ShockTubeMGNEvaluator:
    VAR_NAMES = SHOCKTUBE_VAR_NAMES

    def __init__(self, model, device='cpu', norm_metadata=None, sf=2):
        self.model = model
        self.device = device
    ...
    def generate_rollout(self, simulation, rollout_steps):
        ...
        first = simulation[0]
        static = first.x[:, :sf]
        current = extract_dynamic(first.x, sf)
        edge_index = first.edge_index
        pos = first.pos if hasattr(first, 'pos') and first.pos is not None else static
        edge_features = self.model.compute_edge_features(pos, edge_index)
        gp = extract_global_params(first)

        preds = []
        for step in range(rollout_steps):
            node_features = torch.cat([static, current, gp], dim=-1)
    ...
    def generate_snapshot(self, simulation, num_steps):
        ...
        for step in range(num_steps):
            d = simulation[step]
            gt = extract_dynamic(d.x, sf)
            gp = extract_global_params(d)
            node_features = torch.cat([static, gt, gp], dim=-1)
            delta = self.model(node_features, edge_features, edge_index)
        ...
        with torch.no_grad():
            for si, sim in enumerate(tqdm(simulations, desc=f"{mode.capitalize()} eval")):
                try:
                    ...
                    if not pn: continue
                    tn = [apply_skip(sim[t].y).cpu().numpy() for t in range(len(pn))]
...
def main():
    p = argparse.ArgumentParser(description="MeshGraphNet Shock Tube Evaluation")
    p.add_argument("--model_path", required=True)
    grp = p.add_mutually_exclusive_group(required=True)
    grp.add_argument("--test_dir", type=str)
    grp.add_argument("--test_files", type=str, nargs='+')
    p.add_argument("--output_dir", default="./eval_shocktube_mgn")
    ...
    print(f"\n{'='*70}\nMeshGraphNet SHOCK TUBE EVALUATION\n{'='*70}")
    ...
    input_dim = config.get('input_dim', sf + NUM_USED_DYNAMIC + 3)  # 2 + 3 + 3 = 8
    output_dim = config.get('output_dim', NUM_USED_DYNAMIC)  # 3
    ...
    evaluator = ShockTubeMGNEvaluator(model, device, norm_metadata=norm_metadata, sf=sf)
    ...
    def _run(mode):
        ...
        plot_shocktube_dashboard(evaluator.simulation_metrics,
                                  ...
                                  model_name='MeshGraphNet', eval_mode=mode)
        ...
        if args.create_gifs:
            ...
            for idx in selected:
                ...
                create_shocktube_visualizations(
                    ...
                    si, output_path, model_name='MeshGraphNet',
