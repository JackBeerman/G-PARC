#!/usr/bin/env python3
"""
MeshGraphKAN Elastoplastic Evaluation Script
=============================================
...
from models.meshgraphkan import (
    MeshGraphKAN, MeshGraphKANElastoRollout, build_meshgraphkan,
)
...
class MeshGraphKANEvaluator:
    """Evaluator for MeshGraphKAN elastoplastic models."""
    ...
    def generate_rollout(self, simulation, rollout_steps):
        ...
        for step in range(rollout_steps):
            ...
            edge_features = self.model.compute_edge_features(pos, data_t.edge_index)
            pred_delta = self.model.model(node_features, edge_features, data_t.edge_index)
    ...
    def generate_snapshot_predictions(self, simulation, num_steps):
        ...
        for step in range(num_steps):
            ...
            pos = data_t.pos if hasattr(data_t, 'pos') and data_t.pos is not None else data_t.x[:, :2]
            node_features = torch.cat([data_t.x[:, :sf], F_gt], dim=-1)
            edge_features = self.model.compute_edge_features(pos, data_t.edge_index)
            pred_delta = self.model.model(node_features, edge_features, data_t.edge_index)
...
def evaluate_meshgraphkan(args):
    ...
    config = json.load(open(model_dir / "config.json")) if (model_dir / "config.json").exists() else {
        'hidden_dim': args.hidden_dim, 'processor_size': args.processor_size,
        'num_harmonics': args.num_harmonics, 'aggregation': args.aggregation,
        'mlp_activation': args.mlp_activation, 'num_static_feats': args.num_static_feats,
    ...
    print(f"\n{'='*60}\nMeshGraphKAN ELASTOPLASTIC EVALUATION\n{'='*60}")
    print(f"  Device: {device}, Denorm: {denorm_method}")

    model = build_meshgraphkan(config, device, domain='elasto')
    ...
    def _run_mode(mode):
        ...
        metrics.update({'eval_mode': mode, 'model': 'MeshGraphKAN', 'parameters': total_params})
        ...
        plot_elasto_dashboard(evaluator.simulation_metrics, output_path / f'{mode}_dashboard.png',
                              model_name='MeshGraphKAN', norm_stats=norm_stats, eval_mode=mode)
...
def main():
    p = argparse.ArgumentParser(description="MeshGraphKAN Elastoplastic Evaluation")
    p.add_argument("--model_path", required=True)
    p.add_argument("--test_dir", required=True)
    p.add_argument("--output_dir", default="./eval_meshgraphkan")
    ...
    p.add_argument("--processor_size", type=int, default=4)
    p.add_argument("--num_harmonics", type=int, default=5)
    p.add_argument("--aggregation", default="sum")
    p.add_argument("--mlp_activation", default="relu")
    p.add_argument("--num_static_feats", type=int, default=2)
    p.add_argument("--num_dynamic_feats", type=int, default=2)
    p.add_argument("--num_layers_node_processor", type=int, default=2)
    p.add_argument("--num_layers_edge_processor", type=int, default=2)
    p.add_argument("--num_layers_edge_encoder", type=int, default=2)
    p.add_argument("--num_layers_node_decoder", type=int, default=2)
    ...
    evaluate_meshgraphkan(main_args)
