...
class MeshGraphKANRollout(nn.Module):
    ...
    def forward(self, sequence, dt=1.0, teacher_forcing_ratio=0.0):
        ...
        Loss should be: MSE(pred_delta, data.y - current_state_at_t).
        ...
        Returns:
            ...
            input_states: list of current_dynamic used as input at each timestep
                          (needed to compute delta targets in the loss)
        """
        predictions = []
        input_states = []
        ...
        for t, data in enumerate(sequence):
            ...
            # Save the input state BEFORE prediction (for delta target computation)
            input_states.append(current_dynamic.clone())

            # Network output is predicted DELTA (change from current to next state)
            pred_delta = self.model(node_features, edge_features, data.edge_index)
            predictions.append(pred_delta)

            if t < len(sequence) - 1:
                if torch.rand(1).item() < teacher_forcing_ratio:
                    # Teacher forcing: use GT next state as input for next step
...
def train_epoch(model, train_loader, optimizer, device, epoch, total_epochs, args):
    ...
    for seq in pbar:
        ...
        for t, (pred_delta, input_state, data) in enumerate(zip(predictions, input_states, seq)):
            # Delta target: how much the state should change
            target_delta = data.y - input_state

            if args.mask_eroding and hasattr(data, 'elements') and hasattr(data, 'x_element'):
                step_loss, nv, nm = compute_masked_loss(
                    pred_delta, target_delta, data.elements, data.x_element,
