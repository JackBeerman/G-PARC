...
using the KarmanVortexRolloutDataset class. Optimized for HPC environments with
...
def main():
    ...
    train_dataset = KarmanVortexRolloutDataset(
        directory=train_dir,
    ...
    val_dataset = KarmanVortexRolloutDataset(
        directory=val_dir,
    ...
    test_dataset = KarmanVortexRolloutDataset(
        directory=test_dir,
    ...
    # Right after train_loader = DataLoader(...)
    #print("\n" + "="*60)
    #print("DEBUGGING DATASET")
    #print("="*60)
    #print(f"train_dataset.simulation_ids: {train_dataset.simulation_ids}")
    #print(f"Number of simulations: {len(train_dataset.simulation_ids)}")
    #print(f"seq_len: {train_dataset.seq_len}")
    #print(f"stride: {train_dataset.stride}")
    
    # Try to manually iterate
    #print("\nTrying manual iteration...")
    count = 0
    for seq in train_dataset:
        #print(f"Got sequence {count}: length={len(seq)}")
        if count >= 2:
            break
        count += 1
