...
UPDATED: Support for global max normalization (matches training script)
UPDATED: Added working SNAPSHOT evaluation mode
"""
...
class ElastoPlasticEvaluator:
    ...
    def _ensure_mesh_cached(self, initial_data, mesh_id_int):
        """No-op for PARC v1 — no MLS operators to cache."""
        pass

    def generate_rollout(self, initial_data, simulation, rollout_steps):
        """Generate autoregressive rollout predictions for PARC v1."""
        predictions = []
        
        # Extract static features and compute learned features ONCE
        static_feats = simulation[0].x[:, :self.model.num_static_feats]
        edge_index = simulation[0].edge_index
        learned_static_features = self.model.feature_extractor(static_feats, edge_index)
        ...
        for step in range(rollout_steps):
            ...
            # Derivative + Integration (same as model.forward inner loop)
            Fdot_input = torch.cat([learned_static_features, F_prev], dim=-1)
    ...
    def generate_snapshot_predictions(self, simulation, num_steps):
        """
        Generate snapshot (single-step) predictions for PARC v1.
        ...
        for step in range(num_steps):
            ...
            # Derivative + Integration from GT
            Fdot_input = torch.cat([learned_static_features, F_gt], dim=-1)
            Fdot = self.model.derivative_solver(Fdot_input, edge_index)
            Fint = self.model.integral_solver(Fdot, edge_index)
            F_pred = F_gt + Fint
...
def main():
    ...
    parser.add_argument("--rollout_steps", type=int, default=39)
