#!/usr/bin/env python3
"""
HPC Training Script for GPARC Baseball Model
==============================================

This script trains the integrated GPARC model on baseball pitching data
using the BaseballPitchRolloutDataset class. Optimized for HPC environments with
proper argument parsing, logging, and checkpointing.

Usage:
    python baseball_modularized.py --train_dir /path/to/train --val_dir /path/to/val --test_dir /path/to/test --epochs 50 --lr 1e-4
    
    # With multi-step context (recommended for pose prediction)
    python baseball_modularized.py --train_dir /path/to/train --val_dir /path/to/val --test_dir /path/to/test --epochs 50 --lr 1e-4 --num_context_steps 3
    
For SLURM submission:
    sbatch --gres=gpu:1 --time=24:00:00 python baseball_modularized.py
"""
...
# Import dataset class (you'll need to create this)
from data.baseballData import BaseballPitchRolloutDataset, get_simulation_ids
...
def create_model_with_configurable_solvers(args):
    """Create baseball model with configurable solver architectures optimized for physics learning."""
    
    # The global embedding dimension from GlobalParameterProcessor is 64
    global_embed_dim = 64

    feature_extractor = FeatureExtractorGNN(
        in_channels=args.num_dynamic_feats,  # 9 for baseball (pos + vel + angles)
...
def main():
    parser = argparse.ArgumentParser(description="Train GPARC model on baseball pitching data")
    ...
    parser.add_argument("--num_static_feats", type=int, default=0,
                        help="Number of static features (0 for baseball - all dynamic)")
    parser.add_argument("--num_dynamic_feats", type=int, default=9,
                        help="Number of dynamic features (9 for baseball: pos + vel + angles)")
    parser.add_argument("--skip_dynamic_indices", type=int, nargs='+', default=[],
                        help="Indices of dynamic features to skip (0-based)")
    ...
    # Output arguments
    parser.add_argument("--output_dir", type=str, default="./outputs_baseball",
    ...
    # Log arguments
    logger.info("=" * 80)
    logger.info("BASEBALL GPARC TRAINING")
    logger.info("=" * 80)
    logger.info("\nTraining Arguments:")
    ...
    train_dataset = BaseballPitchRolloutDataset(
        directory=train_dir,
    ...
    val_dataset = BaseballPitchRolloutDataset(
        directory=val_dir,
    ...
    test_dataset = BaseballPitchRolloutDataset(
        directory=test_dir,
    ...
    logger.info(f"\nDataset created - sequence length: {args.seq_len}")
    logger.info(f"  Static features: {args.num_static_feats}")
    logger.info(f"  Dynamic features: {args.num_dynamic_feats}")
    ...
    logger.info(f"  Global features: 1 (pitch_speed)")
    logger.info(f"  Dynamic features: {args.num_dynamic_feats} (pos + vel + angles)")
    ...
    with open(output_dir / "model_summary.txt", "w") as f:
        ...
        f.write(f"\n  Global features: 1 (pitch_speed)")
        f.write(f"\n  Dynamic features: {args.num_dynamic_feats} (3 pos + 3 vel + 3 angles)")
    ...
    logger.info("=" * 80)
    best_model_path = output_dir / "baseball_pitch_best_model.pth"
    
    if args.num_context_steps > 0:
        # Use multi-step context training (recommended for pose prediction)
        logger.info(f"\nUsing multi-step context training with {args.num_context_steps} initial timesteps")
        ...
        logger.info("\nUsing single-step training (give t=0, predict full sequence)")
    ...
    # Plot results
    plot_path = output_dir / "baseball_pitch_loss_curves.png"
    plot_loss_curves(
        train_losses, 
        val_losses, 
        title="Baseball Pitch Training & Validation Loss",
