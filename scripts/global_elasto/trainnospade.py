...
from differentiator.nospadeelasto import ElastoPlasticDifferentiatorNoSPADE as ElastoPlasticDifferentiator
...
def create_model(args, sample_data, norm_stats):
    ...
    print(f"\nCreating model (displacement only)...")
...
def train_epoch(model, train_loader, optimizer, device, epoch, total_epochs, args):
    ...
    for seq in pbar:
        ...
        predictions = model(seq, dt=1.0, teacher_forcing_ratio=teacher_forcing_ratio)
...
def main():
    ...
    print("\n" + "="*70)
    print("G-PARC TRAINING - SCHEDULED SAMPLING")
    print("="*70)
    print(f"Device: {device}")
    print(f"Output: {output_dir}")
    print(f"Normalization: {norm_stats.get('normalization_method', 'unknown')}")
    print(f"")
    print(f"Feature Extractor: GraphConv V2")
    print(f"  Layers: {args.num_layers}")
    print(f"  Hidden: {args.hidden_channels}")
    print(f"  Output: {args.feature_out_channels}")
    print(f"")
    ...
    if args.resume:
        ...
        #    print(f"  Fresh optimizer + scheduler (lr={args.lr}, T_max={args.epochs - checkpoint['epoch'] - 1})")
        #    optimizer = AdamW(model.parameters(), lr=args.lr)
        #    scheduler = CosineAnnealingLR(optimizer, T_max=args.epochs - checkpoint['epoch'] - 1, eta_min=args.lr * 0.01)
        if args.fresh_scheduler:
