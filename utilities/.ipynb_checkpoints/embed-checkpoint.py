...
#    """
#    def __init__(self, normalized_shape, global_dim=3):
#        super().__init__()
...
#        self.ln = nn.LayerNorm(normalized_shape, elementwise_affine=False, eps=1e-6)#, eps=1e-6
#        
#    def forward(self, x, global_attrs):
#        """
#        Args:
#            x (Tensor): Input node features of shape [num_nodes, features].
#            global_attrs (Tensor): Global simulation attributes of shape [3].
#        """
#        normalized = self.ln(x)
#        gamma = self.global_to_gamma(global_attrs)  # Shape: [features]
#        beta = self.global_to_beta(global_attrs)    # Shape: [features]
#        
