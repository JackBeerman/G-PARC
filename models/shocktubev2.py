...
from integrator.integrator import ExplicitIntegralGNN
...
class GPARC(nn.Module):
    ...
    Modified for interpretability: delta_t is passed directly to the integrator
    instead of being embedded with other global parameters.
    """
    def __init__(self, feature_extractor, derivative_solver, integral_solver,
                 num_static_feats=2, num_dynamic_feats=3, skip_dynamic_indices=None, 
                 feature_out_channels=128, use_direct_delta_t=True):
        super().__init__()
        ...
        self.use_direct_delta_t = use_direct_delta_t

        if use_direct_delta_t:
            # Only process pressure and density, not delta_t
            global_dim = 2
        else:
            # Original behavior: embed all three global parameters
            global_dim = 3
            
        # Helper modules for processing global parameters
        self.global_processor = GlobalParameterProcessor(global_dim=global_dim, embed_dim=64)
        self.feature_norm = SimulationConditionedLayerNorm(self.feature_out_channels, global_dim=global_dim)
        self.derivative_norm = SimulationConditionedLayerNorm(self.num_dynamic_feats, global_dim=global_dim)
    ...
    def forward(self, data_list):
        ...
        global_pressure = first_data.global_pressure.flatten()[0]
        global_density = first_data.global_density.flatten()[0]
        delta_t = first_data.global_delta_t.flatten()[0]
        
        if self.use_direct_delta_t:
            # Only embed pressure and density for interpretability
            global_attrs = torch.stack([global_pressure, global_density])
        else:
            # Original behavior: embed all three parameters
        ...
        for data in data_list:
            ...
            if self.use_direct_delta_t:
                # Pass delta_t directly to the integrator for interpretability
                Fint = self.integral_solver(Fdot, edge_index, delta_t=delta_t)
            else:
                # Original behavior: integrator learns the time scaling
