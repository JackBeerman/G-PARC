...
class GPARC(nn.Module):
    ...
    def __init__(self, feature_extractor, derivative_solver, integral_solver,
        ...
        self.global_processor = GlobalParameterProcessor(global_dim=3, embed_dim=64)
        self.feature_norm = SimulationConditionedLayerNorm(self.feature_out_channels, global_dim=3)
        self.derivative_norm = SimulationConditionedLayerNorm(self.num_dynamic_feats, global_dim=3)
    ...
    def forward(self, data_list):
        ...
        global_attrs = torch.stack([
            first_data.global_pressure.flatten()[0],
            first_data.global_density.flatten()[0],
            first_data.global_delta_t.flatten()[0]
        ])
